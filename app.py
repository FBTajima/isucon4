...
def attempt_login(login, password):
    """
    ログイン認証
    失敗時と成功時でそれぞれログインログを出力している(DB)
    user_id情報が一致していればログインログにuser_id情報を付加している
    """
    cur = get_db().cursor()
    ...
    # バンされたIPアドレスかどうか(過去10回失敗するとバンされる)
    if ip_banned():
        if user:
...
def last_login():
    # current_user()不要 userを引数に渡せばいい
    user = current_user()
    if not user:
        return None

    cur = get_db().cursor()
    # 今回ログインした1個前のログイン情報を取得
    cur.execute(
